...
/**
 * Options supported by HttpApiService. Keeps method signature small while allowing
 * callers to pass params/headers and override defaults (like withCredentials).
 */
export interface ApiOptions {
  params?: HttpParams | Record<string, any>;
  headers?: HttpHeaders | Record<string, string>;
  withCredentials?: boolean; // override if needed (default: true)
  timeoutMs?: number; // optional per-request timeout
  retry?: number; // optional retry count for idempotent requests
}

/** Standardized API error returned by the service */
...
export class HttpApiService {
  ...
  // Helpers
  // --------------------------
  private buildParams(params?: HttpParams | Record<string, any>): HttpParams | undefined {
  ...
  private buildHeaders(headers?: HttpHeaders | Record<string, string>): HttpHeaders {
    ...
    if (headers instanceof HttpHeaders) {
      headers.keys().forEach((k) => (httpHeaders = httpHeaders.set(k, headers.get(k) ?? '')));
      return httpHeaders;
    }

    Object.keys(headers).forEach((k) => (httpHeaders = httpHeaders.set(k, headers[k])));
    return httpHeaders;
  }
  ...
  private request<T>(method: string, path: string, body?: any, options?: ApiOptions): Observable<T> {
    ...
    return req$.pipe(catchError((err: HttpErrorResponse) => this.handleError(err)));
  }
...
this.api.get('/products', {
  params: { page: 1, size: 20, tags: ['a','b'] }
});

this.api.post('/auth/login', body, { withCredentials: true });

this.api.get('/public/data', { withCredentials: false });
